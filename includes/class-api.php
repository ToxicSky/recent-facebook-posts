...
class RFBP_API
{
    ...
    {
        $result = $this->call("{$this->fb_id}/posts", [
            'fields' => 'id,picture,full_picture,type,from,message,status_type,object_id,name,caption,description,link,created_time,comments.limit(1).summary(true),likes.limit(1).summary(true)',
        ]);
    ...
    private function format_data($data)
    {
        ...
        foreach ($data as $p) {
            ...
            if (!in_array($p->type, ['status', 'photo', 'video', 'link'])) {
                continue;
            }
            ...
            if (isset($p->link)) {
                $post['post_link'] = $p->link;
            }

            if (isset($p->full_picture)) {
                $image = $p->full_picture;
            } else if (isset($p->picture)) {
                $image = $p->picture;
            }
            ...
            switch ($p->type) {
                ...
                case 'link':
                    $post['link_image']       = (isset($image)) ? $image : '';
    ...
    {
        $result = $this->call("{$this->fb_id}/posts", ['fields' => 'name', 'limit' => 1]);
     ...
     */
    private function call($endpoint, array $data = [])
    {
